...
UPDATE_EVERY = 20       # learn every N steps
NUM_UPDATES = 10        # number of learning passes per update
...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed, num_agents=1):
        ...
        ======
            ...
            num_agents (int): number of agents (1 for v1, 20 for v2)
        """
    ...
    def step(self, states, actions, rewards, next_states, dones):
        """Save experience(s) in replay memory, and use random sample from buffer to learn.
        Supports both single and multiple agent experiences.
        """
        # Handle both single agent and multi-agent cases
        if np.array(states).ndim == 1:
            # Single agent: wrap in lists for uniform handling
            states = [states]
            actions = [actions]
            rewards = [rewards]
        ...
        # Learn every update_every steps, if enough samples are available
